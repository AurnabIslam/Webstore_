...
import { getAllProductAttempt, deleteProductAttempt } from "../../store/Actions/productActions";
...
class userDashboard extends React.Component {
  ...
  deleteProductHandler(product_Id) {
    this.props.requestToDeleteProduct(product_Id)
    this.setState({
      products: this.state.products.filter(product => product._id != product_Id)
    });  
  }
  componentDidMount() {
          ...
          <div className="row">
            { 
              ...
              <div className="col-md-4 col-lg-4 col-sm-12" key={product._id}>
                <ProductCard favFunct={true} addTofavoritHandle={this.addTofavoritHandler} deleteProductHandle={this.deleteProductHandler}  productItem={product} />
              </div>)
              )
            }
          </div>
...
const mapDispatchToProps = (dispatch) => {
  return bindActionCreators({
    ...
    requestToDeleteProduct: (productId) => deleteProductAttempt(productId)
  }, dispatch)
}
