...
const APIEndPoint = 'https://aurnab.herokuapp.com';
//************** Add Prodcut***********************
...
export function deleteProductAttempt(productId) {
    return dispatch => {
        fetch(`${APIEndPoint}/dashboard/products/deleteProduct/${productId}`, {
            method: "POST",
            body: JSON.stringify({ bodyData: { productId } }),
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem("token")}`
            }
            ...
            .then(docs => {
                console.log(docs)
                dispatch({
                    type: ProductActions.deleteProductSuccess,
                    payload: productId
                })
            })
            .catch(error => {
                console.log(error)
                dispatch({
                    type: ProductActions.deleteProductError
                })
            });
    };
};
